    ...
    return <>
        <nav className="h-[8vh] sticky top-0 left-0 w-full bg-bg-100 z-10">
            <div className="container mx-auto flex flex-row items-center w-full h-full">
                <Link href="/" className="hover:text-primary-100 transition duration-150 ease-out mx-auto md:mx-0">
                    <ImaanMasjidLogo/>
                </Link>
                    ...
                    <>
                        {
                            ...
                            <div className="flex flex-row gap-12 font-bold text-lg justify-center items-center mx-auto md:mx-0 md:ml-auto">
                                {menuItems.map((item) =>
                                    ...
                                    (<div key={item.key}>
                                        <Link href={item.link}
                                              className="hover:text-primary-100 transition duration-150 ease-in-out cursor-pointer">{item.title}</Link>
                                    </div>))}
                                {adminSignedIn &&
                                    <div>
                                        <Link href="/admin" className="hover:text-primary-100 transition duration-150 ease-in-out cursor-pointer">Admin Page</Link>
                                    </div>}
                                    <Link href="/donate"
