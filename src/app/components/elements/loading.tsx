...
export default function LoadingAnimation(props: {state: boolean, text?: string, size?: Size})
{
    ...
    return <>
            <div role="status" className="absolute top-0 left-0 w-full h-full bg-bg-100 z-20 flex justify-center items-center gap-4">
                <h1 className={`font-semibold ${textSize}`}>{props.text}</h1>
                <svg aria-hidden="true"
                     className={`inline ${animationSize} text-gray-200 animate-spin fill-accent-100`}
