...
export default async function MonthlyTimetable()
{
    ...
    const prayers = (await supabase.from("DailyPrayers").select("*").gte("date", dateToSupabaseDate(firstDate)).lte("date", dateToSupabaseDate(lastDate)).returns<DailyPrayers[]>()).data
    if(prayers.length == 0)
    {
        return <DefaultMessage message="Timetable for this month has not been uploaded" />;
    }
